...
export default function KidsZone() {
    ...
    const [playCounts, setPlayCounts] = useState<Record<string, { playCount: number; lastPlayed: string }>>({});
    ...
    useEffect(() => {
        const fetchPlayCounts = async () => {
            try {
                const res = await axios.get("/games/stats");
    ...
    // Helper to format play count (e.g., 15000 -> "15k+ plays")
    const formatPlayCount = (count: number): string => {
        if (count >= 1000) {
            return `${(count / 1000).toFixed(0)}k+ plays`;
        }
        return `${count} plays`;
    };

    // Handle game card click: increment play count and navigate
    const handleGameClick = async (gameId: string) => {
        // Optimistically update UI
        const current = playCounts[gameId] || { playCount: 0, lastPlayed: new Date().toISOString() };
        const newCount = current.playCount + 1;
        setPlayCounts(prev => ({
            ...prev,
            [gameId]: { ...current, playCount: newCount }
        }));

        // Send increment request to backend (fire and forget)
        try {
            await axios.post(`/games/${gameId}/play`);
        } catch (error) {
            // Rollback on failure
            setPlayCounts(prev => ({
                ...prev,
            ...
            toast({
                title: "Error",
                description: "Could not track game play. Please try again.",
                variant: "destructive"
            });
        }
                ...
                <div className="mb-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {publicGames.map((game) => {
                            ...
                            const playCountData = playCounts[game.id];
                            const playCount = playCountData?.playCount || 0;
                            return (
                                <Card
                                    key={game.id}
